-- 1) Cron do 4º fornecedor a cada 5min (espelha sync-verified)
SELECT cron.unschedule('sync-provider4-5min') WHERE EXISTS (SELECT 1 FROM cron.job WHERE jobname='sync-provider4-5min');
SELECT cron.schedule(
  ...
  SELECT net.http_post(
    ...
    headers := jsonb_build_object(
      ...
      'x-admin-token', (SELECT decrypted_secret FROM vault.decrypted_secrets WHERE name = 'cron_admin_token')
    ),
    body := '{}'::jsonb
  );
  $cron$
...
-- 2) ops_forensics: ignora pedidos de teste do diretor no check de caixa
...
BEGIN
  ...
  -- v248: pedidos de teste do próprio dono não são "venda sem caixa".
  SELECT coalesce(jsonb_agg(jsonb_build_object(
  ...
  WHERE p.status IN ('completed','Enviado') AND t.pedido_id IS NULL
    ...
    AND lower(coalesce(p.instagram_user,'')) NOT LIKE '%fabiano%';
