...
DECLARE
  ...
  v_queue jsonb := '{}'::jsonb;
BEGIN
  -- 1) Robôs agendados + última execução
  ...
  -- 2) Pedidos pagos sem entrega confirmada
  ...
  WHERE p.status IN ('paid','processing','waiting_provision','MARGIN_HOLD','SMM_FAILED')
    AND p.created_at < now() - interval '3 hours';

  -- 3) Pix pendente com cobranca gerada (dinheiro parado / carrinho)
  SELECT coalesce(jsonb_agg(jsonb_build_object(
  ...
  WHERE p.status = 'pending'
    AND p.mercado_pago_id IS NOT NULL
    AND p.created_at > now() - interval '7 days'
    AND p.created_at < now() - interval '30 minutes';

  -- 4) Fila de e-mail (24h)
  SELECT jsonb_build_object(
  ...
  -- 5) Vendas concluidas sem registro no caixa
  SELECT coalesce(jsonb_agg(jsonb_build_object(
  ...
  WHERE p.status IN ('completed','Enviado')
    AND t.pedido_id IS NULL
    AND p.created_at > now() - interval '30 days';

  -- 6) Catalogo: pacotes nao vendaveis ou sem validacao recente
  SELECT jsonb_build_object(
    'total', count(*),
  ...
  res := jsonb_build_object(
    'generated_at', now(),
    'crons', v_crons,
    'pedidos_pagos_sem_entrega', v_stuck,
    'pix_pendente', v_pix,
    'email', v_email,
    'email_queue_depth', v_queue,
    'vendas_sem_caixa', v_caixa,
    'catalogo', v_catalog
  );
...
REVOKE ALL ON FUNCTION public.ops_forensics() FROM PUBLIC, anon;
