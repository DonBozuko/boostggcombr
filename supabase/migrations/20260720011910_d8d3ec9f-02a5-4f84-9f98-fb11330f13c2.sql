-- Colunas para envio de pedido de avaliação pós-entrega
ALTER TABLE public.pedidos
  ADD COLUMN IF NOT EXISTS email_contato text,
  ADD COLUMN IF NOT EXISTS review_email_sent_at timestamp with time zone;

CREATE INDEX IF NOT EXISTS idx_pedidos_review_pending
  ON public.pedidos (created_at)
  WHERE status = 'completed' AND review_email_sent_at IS NULL AND email_contato IS NOT NULL;

-- LGPD: garantir que o e-mail também é anonimizado
