...
const LACUNA_CRUA_RE = /\{\{\s*[\w.]+\s*\}\}|\[\s*inserir[^\]]*\]/i;
/** "undefined"/"null" só valem como falha dentro de texto lido pelo cliente. */
const LACUNA_TEXTO_RE = /\b(?:undefined|NaN)\b/;
...
export function checkEmailCoherence(input: {
  ...
  for (const texto of extrairTextoVisivel(source)) {
    ...
    if (LACUNA_TEXTO_RE.test(texto)) {
      out.push({ template, kind: "lacuna_nao_preenchida", trecho: texto });
    }
  }
  ...
  for (const linha of String(source).split(/\n/)) {
    const m = linha.match(LACUNA_CRUA_RE);
    if (m) {
