...
export const listResellers = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }): Promise<{ ok: boolean; error?: string; resellers: Reseller[] }> => {
    if (!(await (await import("@/lib/admin-guard.server")).assertAdmin(data.token, "resellers")).ok) return { ok: false, error: "UNAUTHORIZED", resellers: [] };
...
export const createReseller = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }): Promise<{ ok: boolean; error?: string; apiKey?: string }> => {
    if (!(await (await import("@/lib/admin-guard.server")).assertAdmin(data.token, "resellers")).ok) return { ok: false, error: "UNAUTHORIZED" };
...
export const resellerLedger = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    if (!(await (await import("@/lib/admin-guard.server")).assertAdmin(data.token, "resellers")).ok) return { ok: false as const, error: "UNAUTHORIZED", rows: [] };
    const { supabaseAdmin } = await import("@/integrations/supabase/client.server");
