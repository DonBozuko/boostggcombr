...
async function authorized(request: Request) {
  ...
  if ((await (await import("@/lib/admin-guard.server")).assertAdmin(token, "route:check-saldo")).ok) return true;
  if ((await (await import("@/lib/admin-guard.server")).assertAdmin(token, "route:check-saldo", { allowCron: true })).ok) return true;
  try {
