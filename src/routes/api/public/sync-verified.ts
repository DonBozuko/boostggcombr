...
async function authorized(request: Request) {
  ...
  if ((await (await import("@/lib/admin-guard.server")).assertAdmin(token, "route:sync-verified")).ok) return true;
  if ((await (await import("@/lib/admin-guard.server")).assertAdmin(token, "route:sync-verified", { allowCron: true })).ok) return true;
  try {
