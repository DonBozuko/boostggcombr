...
export const Route = createFileRoute("/telegram")({
  head: () => {
    const title = "Membros para Grupo e Canal do Telegram | BoostGG";
    const description =
      "Cresça seu grupo e canal do Telegram com membros via Pix. Entrega automática em minutos, pacotes 🌎 Global de alto volume.";
    const url = "https://www.boostgg.com.br/telegram";
    const ogImage = `https://www.boostgg.com.br${ogTelegram}?v=48`;
    return {
      meta: [
      ...
      scripts: [buildProductJsonLd({ network: "Telegram", category: "Membros", url, description }), buildFaqJsonLd(FAQS["telegram"])],
    };
  },
  ...
  const dispatchPedido = async (selected: Plan, profileValue: string, bumpUpgrade: boolean) => {
    ...
    try {
      ...
      trackInitiateCheckout({
        ...
        contentName: `${selected.tier} telegram`,
      });
      setPaid(false);
      ...
      <header className="sticky top-0 z-50 bg-black/90 border-b transition-all duration-300" style={{ borderColor: `${AERO}66` }}>
        <div className="container mx-auto px-6 h-16 flex items-center justify-between">
          <div className="flex items-center gap-2">
            <BrandHeader subtitle="Membros para Telegram via Pix" />
          </div>
          <ShowcaseTrigger />
        </div>
      </header>
      <h1 className="text-center text-2xl sm:text-3xl font-bold text-white mt-4 mb-2 px-4">
        Crescer Telegram — Membros para Grupo e Canal
      </h1>
      <ShowcaseShell>
      ...
      {planId && (<section id="tg-pedido" className="py-12 border-y" style={{ borderColor: `${AERO}44`, background: "#0d0d0e" }}>
        <div className="container mx-auto px-4 sm:px-6 max-w-xl">
          ...
          <div className="mt-6 rounded-2xl p-4 space-y-3"
            ...
            <div className="space-y-2">
              <Label htmlFor="tg-email">E-mail para comprovante e status <span className="text-zinc-500 text-xs">(obrigatório)</span></Label>
              <Input
                id="tg-email"
                type="email"
                value={email}
